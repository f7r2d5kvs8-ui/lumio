...
function renderAuth() {
  ...
  if (choosing) {
    root.innerHTML = `<main class="screen account-screen"><section class="hero auth-choice"><div class="eyebrow">Lumio</div><h1>${t.welcome}</h1><p>${t.beginJourney}</p>${googleLoginButton(googleText)}<p class="auth-message google-auth-message" id="auth-message" aria-live="polite"></p><div class="account-choices"><button class="choice-button guest-choice" data-action="guest"><span class="choice-icon">▶</span><span><strong>${t.playGuest}</strong><small>${t.beginNow}</small></span></button><button class="choice-button account-choice" data-action="signup"><span class="choice-icon">★</span><span><strong>${t.createAccount}</strong><small>${t.saveProgress}</small></span></button></div><button class="login-link" data-action="login">${t.haveAccount} <strong>${t.logIn}</strong></button></section></main>`;
    ...
    root.querySelector('[data-action="guest"]').onclick = () => { cloudUser = null; profile.account = null; profile.guest = true; saveProfile(profile); view = returnView || (profile.childName ? (profile.selectedLanguage ? (profile.selectedGame ? selectedGameView() : 'games') : 'languages') : 'child-name'); returnView = null; render(); }; return;
  }
...
function header() { const t = copy(); return `<header class="topbar"><button class="brand" data-action="hub" aria-label="${navCopy().home}">Lu<span>mio</span></button><div class="stat-row"><button class="chip child-account" data-action="child-name" aria-label="${t.changeChildName}">👤 <span>${escape(profile.childName || t.child)}</span></button><button class="chip" data-action="templates" aria-label="${navCopy().style}">🎨</button>${soundButton(false)}<button class="chip" data-action="parent" aria-label="${t.parents}">👨‍👩‍👧</button><span class="chip">🔥 ${profile.rewards.streak || 0}</span><span class="chip">⭐ ${profile.rewards.stars || 0}</span></div></header>`; }
