...
async function syncMathProgress() { if (!cloudUser) return; const rows = (await readProgress(cloudUser.id)).filter(row => row.level > MATH_LEVEL_OFFSET && row.level <= MATH_LEVEL_OFFSET + 10); if (!rows.length) return; const levels = { ...getMathProgress().levels }; rows.forEach(row => { const id = row.level - MATH_LEVEL_OFFSET; const current = levels[id] || {}; levels[id] = { round: Math.max(current.round || 0, row.word_index || 0), stars: current.stars || 0, complete: Boolean(current.complete || row.completed) }; }); saveMathProgress({ levels }); }
async function persistMathProgress(levelId) { if (!cloudUser) return; const progress = mathLevelProgress(levelId); await writeProgress(cloudUser.id, MATH_LEVEL_OFFSET + levelId, progress.round || 0, Boolean(progress.complete)); }
...
function renderNameTracing() {
  ...
  const letterMap = nameLetters.map((letter, index) => `${nameEntries[index]?.wordBreakBefore ? '<span class="name-progress-gap" aria-hidden="true"></span>' : ''}<span class="name-progress-letter" data-name-letter="${index}">${rtl || customPractice ? letter : (index === 0 ? letter.toUpperCase() : letter)}</span>`).join('');
  ...
  root.innerHTML = `${header()}<main class="screen tracing-screen"><div class="game-head"><button class="back" data-action="letters">← ${t.letters}</button><span class="count">${customPractice ? t.customTrace : t.myName}</span></div><section class="tracing-card"><div class="eyebrow">${trailTitle}</div><h1>${t.write}: <span dir="${nameDirection}">${escape(nameValue)}</span></h1><p>${intro}</p><div class="name-progress" dir="${nameDirection}" aria-label="${escape(nameValue)}">${letterMap}</div><div class="trace-stage name-stage"><svg id="trace-svg" class="name-svg" style="width:${width}px" viewBox="0 0 ${width} 500" role="img" aria-label="${t.write} ${escape(nameValue)}">${strokes}<circle id="trace-start" class="trace-marker start" r="13"/><circle id="trace-end" class="trace-marker end" r="13"/><circle id="trace-dot" class="trace-dot" r="17" tabindex="0" role="button" aria-label="${startPrompt}"/></svg></div><p id="trace-feedback" class="feedback">${startPrompt}</p><div class="controls"><button class="button soft" data-action="listen-trace">🔊 ${t.listen}</button><button class="button primary" data-action="retry-trace">${t.retry}</button></div></section></main>${adBanner()}`;
  bindHeader();
  root.querySelector('[data-action="letters"]').addEventListener('click', () => { tracingSession = null; view = 'letters'; render(); });
...
function renderAuth() {
  ...
  if (choosing) {
    ...
    root.querySelector('[data-action="guest"]').onclick = () => { cloudUser = null; profile.account = null; profile.guest = true; saveProfile(profile); view = profile.childName ? (profile.selectedLanguage ? (profile.selectedGame ? selectedGameView() : 'games') : 'languages') : 'child-name'; render(); }; return;
  }
...
async function boot() { if (!profile.appLanguage) { registerServiceWorker(); view = 'app-language'; render(); return; } try { const sessionState = await currentSession(); cloudUser = sessionState?.user || null; if (cloudUser) { profile.account = { email: cloudUser.email, provider: 'supabase' }; profile.guest = false; await syncCloudProgress(); await syncTracingProgress(); await syncMathProgress(); view = profile.childName ? (profile.selectedLanguage ? (profile.selectedGame ? selectedGameView() : 'games') : 'languages') : 'child-name'; } else if (profile.guest) { profile.account = null; view = profile.childName ? (profile.selectedLanguage ? (profile.selectedGame ? selectedGameView() : 'games') : 'languages') : 'child-name'; } else { profile.account = null; view = 'auth'; } saveProfile(profile); } catch (error) { console.warn('Cloud session unavailable; offline mode remains available.', error); if (profile.guest) view = profile.childName ? (profile.selectedLanguage ? (profile.selectedGame ? selectedGameView() : 'games') : 'languages') : 'child-name'; } registerServiceWorker(); render(); }
...
function render() { applyTemplate(); setDocumentLanguage(profile.appLanguage || 'nl'); if (view === 'app-language') renderAppLanguage(); else if (view === 'auth') renderAuth(); else if (view === 'child-name') renderChildName(); else if (view === 'native-name') renderNativeName(); else if (view === 'languages') renderLanguages(); else if (view === 'games') renderGames(); else if (view === 'templates') renderTemplates(); else if (view === 'number-house-levels') renderNumberHouseLevels(); else if (view === 'number-houses') renderNumberHouses(); else if (view === 'letters') renderLetters(); else if (view === 'custom-tracing-input') renderCustomTracingInput(); else if (view === 'tracing') renderTracing(); else if (view === 'game') renderGame(); else if (view === 'parent') renderParent(); else renderHome(); decorateWithMascot(); if (!root.querySelector('.release-tag')) root.insertAdjacentHTML('beforeend', `<span class="release-tag">v${RELEASE}</span>`); }
