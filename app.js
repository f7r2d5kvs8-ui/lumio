...
function setupTracing() {
  ...
  const activateStroke = () => {
    ...
    tracingSession.samples = samples; tracingSession.length = length; tracingSession.furthest = 0; place(dot, samples[0]); place(start, samples[0]); place(end, samples[samples.length - 1]); progress.style.strokeDasharray = String(length); progress.style.strokeDashoffset = String(length);
  };
  activateStroke();
  ...
  const update = index => { const progress = progresses[tracingSession.strokeIndex]; tracingSession.furthest = Math.max(tracingSession.furthest, index); place(dot, tracingSession.samples[index]); progress.style.strokeDashoffset = String(tracingSession.length * (1 - tracingSession.furthest / (tracingSession.samples.length - 1))); if (!tracingSession.completed && tracingSession.furthest >= tracingSession.samples.length - 8) { if (tracingSession.strokeIndex < shadows.length - 1) { tracingSession.strokeIndex += 1; root.querySelector('#trace-feedback').textContent = 'Goed zo! Volg nu de volgende lijn.'; activateStroke(); } else finishTracing(); } };
