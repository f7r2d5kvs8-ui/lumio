...
const appLanguagePageCopy = {
  nl: { title:'Kies de taal van Lumio', intro:'Deze taal gebruiken we voor knoppen en uitleg.', back:'Terug naar welkom' },
  en: { title:'Choose Lumio’s language', intro:'We’ll use this language for buttons and instructions.', back:'Back to welcome' },
  fa: { title:'زبان لومیو را انتخاب کنید', intro:'دکمه‌ها و راهنمایی‌ها به این زبان نمایش داده می‌شوند.', back:'بازگشت به خوش‌آمدید' }
};
const systemAppLanguage = () => {
  const preferred = (navigator.languages?.[0] || navigator.language || 'en').toLowerCase();
  if (preferred.startsWith('nl')) return 'nl';
  if (preferred.startsWith('fa')) return 'fa';
  return 'en';
};
...
function renderAuth() {
  ...
  if (choosing) {
    const appLanguage = languageCatalog.find(language => language.id === profile.appLanguage);
    root.innerHTML = `<main class="screen account-screen"><section class="hero auth-choice"><button class="welcome-language" data-action="choose-app-language" aria-label="${navCopy().appLanguage}"><span aria-hidden="true">🌐</span> ${appLanguage.nativeName}</button><div class="eyebrow">Lumio</div><h1>${t.welcome}</h1><p>${t.beginJourney}</p><div class="account-choices"><button class="choice-button guest-choice" data-action="guest"><span class="choice-icon">▶</span><span><strong>${t.playGuest}</strong><small>${t.beginNow}</small></span></button><button class="choice-button account-choice" data-action="signup"><span class="choice-icon">★</span><span><strong>${t.createAccount}</strong><small>${t.saveProgress}</small></span></button></div><button class="login-link" data-action="login">${t.haveAccount} <strong>${t.logIn}</strong></button><p><a class="policy-link" href="privacy.html" target="_blank" rel="noopener">${safety().privacy}</a></p></section></main>`;
    root.querySelector('[data-action="choose-app-language"]').onclick = () => { languageTarget = 'welcome'; view = 'app-language'; render(); };
